...
for line in data:
    line = line.split()
    instruction = Instruction(op=line[0], value=int(line[1]))
...
def run(instructions):
    ...
    while True:
        ...
        if current_instruction.op == 'nop':
            ...
            if (index + current_instruction.value) in visited:
...
def run_modify():
    for index in range(len(instructions)):
        ...
        if modified_instructions[index].op in ['jmp', 'nop']:
            modified_instructions[index].op = 'nop' if modified_instructions[index].op == 'jmp' else 'jmp'
        else:
            continue
        halted, accumulator = run(modified_instructions)
        if halted:
            return accumulator


print(f'Part one: {run(instructions)[1]}')
print(f'Part two: {run_modify()}')
