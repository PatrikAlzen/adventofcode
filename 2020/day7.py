    ...
    for target in targets:
        for bag in graph:
            if target in graph[bag] and bag not in all_parents:
                new_parents.add(bag)
    all_parents = all_parents.union(new_parents)
    targets, new_parents = new_parents, set()

print(f'Part one: {len(all_parents)}')


def count_bags(target):
    total = 1
    for bag, contents in graph.items():
        if target in bag:
            for item, count in contents.items():
                total += int(count) * count_bags(item)

    return total


print(f"Part two: {count_bags('shiny gold') - 1}")
