...
def split_items(row):
    ...
    for item in row.split():
        k, v = item.split(':')
        passport_part[k] = v
    return passport_part

def byr(data):
    return re.match('^\d\d\d\d$', data) and (1920 <= int(data) <= 2002)

def iyr(data):
    return re.match('^\d\d\d\d$', data) and (2010 <= int(data) <= 2020)

def eyr(data):
    return re.match('^\d\d\d\d$', data) and (2020 <= int(data) <= 2030)

def hgt(data):
    try:
        height, unit = re.match('(\d+)(in|cm)', data).groups()
    except:
        return False
    if unit == 'cm':
...
def valid_data(passport):
    return all([checks[key](passport[key]) for key in passport.keys()])

def valid_fields(passport, required_fields):
    return all([passport.get(key, False) for key in required_fields])
...
for passport in passports:
    if valid_fields(passport, required):
        fields_valid.append(passport)
        if valid_data(passport):
            data_valid.append(passport)

print(f'Part one: {len(fields_valid)}')
print(f'Part two: {len(data_valid)}')
